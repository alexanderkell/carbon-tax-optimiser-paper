...
Averting the effects of anthropogenic climate change requires a transition from fossil-fuels to low-carbon technology. A primary method of achieving this is to decarbonise the electricity grid to provide energy to other sources such as automotive and heating.

%Aims

In this paper we aim to find optimal carbon tax policies by using a genetic algorithm approach to an electricity market model. To do this we use the NSGA-II genetic algorithm to minimise average electricity price and relative carbon intensity of the electricity mix. 

%Methodology

%Results

We show that it is possible find a range of carbon taxes to suit different preferences, whilst minimising both electricity cost and carbon intensity.
...
We explore the electricity mix generated of three different strategies, which are shown in Figure \ref{fig:highlighted_linear_actual_pdcs}. We selected the highest, lowest and the lowest flat carbon strategy to show a range of possible strategies.
...
Whilst there does not seem to be a significant difference, with solar providing ${\sim}60\%$ of electricity mix by 2035, there is an observable difference with the other electricity generator types.
